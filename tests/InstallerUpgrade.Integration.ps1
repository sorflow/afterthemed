...
try {
    ...
    Build-TestInstaller '1.3.5' 'AfterThemed-Setup-old'
    Build-TestInstaller '1.3.6' 'AfterThemed-Setup-new'
    ...
    if ($oldRegistration.DisplayVersion -ne '1.3.5') {
        throw "Expected the synthetic old registration to be 1.3.5; got $($oldRegistration.DisplayVersion)."
    }
    ...
    if ($newRegistration.DisplayVersion -ne '1.3.6') {
        throw "Expected the upgraded registration to be 1.3.6; got $($newRegistration.DisplayVersion)."
    }
    ...
    if (-not (Select-String -LiteralPath $setupLog -SimpleMatch 'Removing AfterThemed 1.3.5 before installing 1.3.6.' -Quiet)) {
        throw 'The setup log does not show the previous-version uninstall path.'
    }

    Write-Host 'PASS: installer removed 1.3.5, installed 1.3.6, and left one current uninstaller.'
}
finally {
